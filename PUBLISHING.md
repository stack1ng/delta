...
- npm package: `@pyro/delta`
- public GitHub repository: `stack1ng/pyro-delta`
...
At the time this workflow was added, `@pyro/delta` was not present in the public
registry. Bootstrap `0.1.0` once with an interactive maintainer session—never a
CI or long-lived automation token:

1. Create the **public** `stack1ng/pyro-delta` repository, push this history,
   and confirm `package.json` still names that exact repository.
2. Confirm the maintainer can publish public packages in the npm `@pyro` scope
...
3. From a clean checkout, enter `nix develop` and run:
   ...
   npm login
   npm publish /tmp/pyro-delta-0.1.0.tgz --access public
   npm logout
   ```
...
4. On npm, open `@pyro/delta` → Settings → Trusted Publisher and configure:
   GitHub owner `stack1ng`, repository `pyro-delta`, workflow `release.yml`,
...
the npm provenance links back to `stack1ng/pyro-delta` and `release.yml`.
